...
interface ComplaintFileUploadProps {
  activationCode: string;
}

export default function ComplaintFileUpload({ activationCode }: ComplaintFileUploadProps) {
  ...
  const handleAnalyzeSSE = useCallback(async (caseId: string) => {
    ...
    try {
      const analyzeRes = await fetch('/api/complaint/analyze', {
        method: 'POST',
          ...
          for (const line of raw.split('\n')) {
            ...
            try {
              ...
              if (event.type === 'status') {
                ...
                setProgress(100);
                setStage('done');
                setTimeout(() => router.push(`/complaint/${caseId}`), 500);
                return;
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const createRes = await fetch('/api/complaint/cases', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [handleAnalyzeSSE, activationCode]);
  ...
  }, [textContent, handleAnalyzeSSE, activationCode]);
      ...
      {inputMode === 'text' && (
        <div className="space-y-3">
          <textarea
            ...
            placeholder="请将 12315 投诉通知内容粘贴到此处（包含消费者诉求、投诉事由等）..."
            value={textContent}
